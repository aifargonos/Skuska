...
\begin{frame}{What is Answer-Set Programming}
	\onslide<+->
	
	\begin{itemize}
		\item Declarative problem solving:
	...
	% 		\fill [red] (4) circle (2pt);
		...
		\item An \structure{answer-set program} is a set of rules $r$ of the form:
		\begin{center}
		$h\ \code{:-}\ b_1 \code{,} \ldots \code{,}\ b_n \code{,}\ \code{not}\ b_{n+1} \code{,} \ldots \code{,}\ \code{not}\ b_m \code{.}$
		\end{center}
		\begin{itemize}
			\item where $h, b_1, \ldots, b_m$ are atoms
			\item $\head{r} = \set{h}$ is \structure{head}, $\body{r} = \set{b_1, \ldots, \code{not}\ b_m}$ is \structure{body}
			\item $\pbody{r} = \set{b_1, \ldots, b_n}$ and $\nbody{r} = \set{b_{n+1}, \ldots, b_m}$
		\end{itemize}
		
		\onslide<+->
		
		\item Semantics is defined over propositional rules
		\item Rules with variables are grounded by applying all possible substitutions
	\end{itemize}
...
\begin{frame}{Example Problem}
	...
	\begin{itemize}
		\item Having a graph: \hspace{1cm}
			\begin{tikzpicture} [
		...
		\item has grounding: %\gr{\prog_{r,4}}:
			\begin{columns}[t,totalwidth=0.94\textwidth]
			\column{0.565\textwidth}
			{\footnotesize
				\newcommand{\hs}{\hspace{0.125\textwidth}}%
				\hs\code{r(1) :- start(1), e(1,1).}\\
				\hs\code{r(1) :- start(2), e(2,1).}\\
				\hs\code{r(1) :- start(3), e(3,1).}\\
				\hs\ldots\\
				\hs\code{r(2) :- start(1), e(1,2).}\\
				\hs\code{r(2) :- start(2), e(2,2).}\\
				\hs\ldots\\
			}
...
\begin{frame}{Semantics}
	...
	has minimal models \set{p} and {\color{red}\set{q}}.
